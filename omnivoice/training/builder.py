...
def build_model_and_tokenizer(
    ...
    if config.init_from_checkpoint:
        ...
        logger.info(f"Loading base LLM {config.llm_name_or_path} (Attn: {attn_impl})")
...
def build_dataloaders(
    ...
    raw_train_ds = WebDatasetReader(manifests=train_manifests, evaluation=False, repeat=True)
    ...
    import sys
    nw = config.num_workers if sys.platform != "win32" else min(config.num_workers, 2)
